...
public class CandlashItem extends WhipItem {
	...
	public CandlashItem(Tier tier, float attackDamage, float attackSpeed, Properties properties) {
	...
	public void doPostHurtEffects(@Nullable Whip whip, Entity entity) {
		if (entity.level() instanceof ServerLevel serverLevel) {
			...
			serverLevel.sendParticles(
				ESExplosionParticleOptions.fromIntColor(
					...
					PARTICLE_COLORS.get(entity.level().getRandom().nextInt(PARTICLE_COLORS.size())),
					PARTICLE_COLORS.get(entity.level().getRandom().nextInt(PARTICLE_COLORS.size())),
					0.4f
				),
				x, y, z,
			...
			for (int i = 0; i < 4; i++) {
				Vec3 speed = new Vec3(
					(entity.level().getRandom().nextFloat() - entity.level().getRandom().nextFloat()) * 0.1F,
					entity.level().getRandom().nextFloat() * 0.05F,
					(entity.level().getRandom().nextFloat() - entity.level().getRandom().nextFloat()) * 0.1F
				).normalize();
		...
		if (entity.level().getRandom().nextFloat() < 0.75) {
			if (entity instanceof LivingEntity living) {
				living.addEffect(new MobEffectInstance(ESMobEffects.FLAMMABLE.get(), 200, 0));
			}
