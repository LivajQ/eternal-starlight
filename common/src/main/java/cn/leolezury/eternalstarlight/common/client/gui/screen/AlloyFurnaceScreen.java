...
public class AlloyFurnaceScreen extends AbstractContainerScreen<AlloyFurnaceMenu> implements MenuAccess<AlloyFurnaceMenu> {
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
		...
		guiGraphics.blit(
			COOLING_PROGRESS,
			this.leftPos + 85,
			this.topPos + 50 - coolingProgress,
			0,
			16 - coolingProgress,
			16,
			coolingProgress
		...
		guiGraphics.blit(
			OVERHEAT_PROGRESS,
			this.leftPos + 89,
			this.topPos + 34 - overheatProgress,
			0,
			18 - overheatProgress,
			26,
			overheatProgress
	...
	protected void renderBg(GuiGraphics guiGraphics, float partialTick, int mouseX, int mouseY) {
		...
		guiGraphics.blit(CONTAINER_BACKGROUND, x, y, 0, 0, this.imageWidth, this.imageHeight);
		int litProgress = Mth.ceil(this.menu.getLitProgress() * 13.0F) + 1;
		guiGraphics.blit(LIT_PROGRESS, 14, 14, 0, 14 - litProgress, this.leftPos + 10, this.topPos + 50 - litProgress, 14, litProgress);
		int burnProgress = Mth.ceil(this.menu.getBurnProgress() * 24.0F);
		guiGraphics.blit(BURN_PROGRESS, 24, 16, 0, 0, this.leftPos + 89, this.topPos + 18, burnProgress, 16);
		int coolingProgress = Mth.ceil(this.menu.getCoolingProgress() * 15.0F) + 1;
		guiGraphics.blit(COOLING_PROGRESS, 16, 16, 0, 16 - coolingProgress, this.leftPos + 85, this.topPos + 50 - coolingProgress, 16, coolingProgress);
		int overheatProgress = Mth.ceil(this.menu.getOverheatProgress() * 18.0F);
		guiGraphics.blit(OVERHEAT_PROGRESS, 26, 18, 0, 18 - overheatProgress, this.leftPos + 89, this.topPos + 34 - overheatProgress, 26, overheatProgress);
	}
	 */
}