...
public class PetalScytheItem extends ScytheItem implements SwingAttackWeapon {
	...
	public PetalScytheItem(Tier tier, boolean canTill, int damage, float speed, float reach, float sweep, Properties props) {
		super(tier, canTill, damage, speed, reach, sweep, props);
	}
		...
		if (!level.isClientSide && !SpecialItemCooldown.isOnCooldown(entity, this)) {
			...
			for (int i = -2; i <= 2; i++) {
				...
				cloud.setDeltaMovement(
					ESMathUtil.rotationToPosition(1, -entity.getViewXRot(0), entity.getViewYRot(0) + 90)
