...
public class ESCreativeModeTabs {
	...
	private static final Comparator<Holder<PaintingVariant>> PAINTING_COMPARATOR =
		Comparator.<Holder<PaintingVariant>>comparingInt(h -> h.value().getWidth() * h.value().getHeight())
			.thenComparingInt(h -> h.value().getWidth());
	...
	public static void generatePresetPaintings(CreativeModeTab.Output output, HolderLookup.Provider provider, HolderLookup.RegistryLookup<PaintingVariant> registryLookup, Predicate<Holder<PaintingVariant>> predicate) {
		registryLookup.listElements()
			...
			.forEach(reference -> {
				...
				ResourceKey<PaintingVariant> key = reference.unwrapKey().orElse(null);
				if (key != null) {
					CompoundTag entityTag = new CompoundTag();
					entityTag.putString("variant", key.location().toString());
