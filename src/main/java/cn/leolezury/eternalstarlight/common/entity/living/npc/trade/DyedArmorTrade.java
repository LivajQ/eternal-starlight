...
public class DyedArmorTrade implements VillagerTrades.ItemListing {
	...
	public MerchantOffer getOffer(Entity entity, RandomSource randomSource) {
		...
		if (stack.getItem() instanceof DyeableLeatherItem) {
			...
			if (randomSource.nextFloat() > 0.7F) list.add(getRandomDye(randomSource));
			if (randomSource.nextFloat() > 0.8F) list.add(getRandomDye(randomSource));

			stack = DyeableLeatherItem.dyeArmor(stack, list);
		}

		return new MerchantOffer(currency.copy(), stack, this.maxUses, 0, 0);
	}
