...
public class Yeti extends Animal {
	...
	public SpawnGroupData finalizeSpawn(ServerLevelAccessor level, DifficultyInstance difficulty, MobSpawnType spawnType, @Nullable SpawnGroupData spawnGroupData, @Nullable CompoundTag dataTag) {
		...
		this.getAttribute(Attributes.MAX_HEALTH).setBaseValue(ESConfig.yeti.maxHealth.get());
		this.getAttribute(Attributes.ARMOR).setBaseValue(ESConfig.yeti.armor.get());
	...
	public static boolean checkYetiSpawnRules(EntityType<? extends Yeti> type, LevelAccessor level, MobSpawnType spawnType, BlockPos pos, RandomSource random) {
		return level.getBlockState(pos.below()).is(BlockTags.ANIMALS_SPAWNABLE_ON) && ESConfig.yeti.canSpawn.get();
	}
}
