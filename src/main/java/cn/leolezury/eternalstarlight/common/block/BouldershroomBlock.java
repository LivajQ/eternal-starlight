...
public class BouldershroomBlock extends DirectionalBudBlock implements BonemealableBlock {
		...
		for (BlockPos blockPos : stemPositions) {
			...
			if (random.nextInt(8) == 0) {
				...
				for (int i = 1; i <= maxLength; i++) {
					level.setBlock(blockPos.below(i), i == maxLength ? ESBlocks.BOULDERSHROOM_ROOTS.get().defaultBlockState() : ESBlocks.BOULDERSHROOM_ROOTS_PLANT.get().defaultBlockState(), UPDATE_ALL);
				}
			}
		}
	...
	public void performBonemeal(ServerLevel serverLevel, RandomSource random, BlockPos pos, BlockState state) {
		...
		if (!growMushroom(serverLevel, random, pos, state.getValue(FACING))) {
			serverLevel.setBlock(pos, state, UPDATE_ALL);
		}
	}
}
