...
public class CrystalGreatswordItem extends GreatswordItem {
	...
	public CrystalGreatswordItem(Tier tier, int damage, float speed, float reach, Properties props) {
	...
	public boolean hurtEnemy(ItemStack stack, LivingEntity target, LivingEntity attacker) {
		...
		if (existingLevel >= 4) {
			target.removeEffect(ESMobEffects.CRYSTAL_INFECTION.get());
			...
			if (target.level() instanceof ServerLevel serverLevel) {
				for (int i = 0; i < 3; i++) {
					...
					serverLevel.sendParticles(
						ESExplosionParticleOptions.fromIntColor(
							ESParticles.BLAST.get(),
							PARTICLE_COLORS.get(target.getRandom().nextInt(PARTICLE_COLORS.size())),
							PARTICLE_COLORS.get(target.getRandom().nextInt(PARTICLE_COLORS.size())),
							0.5f
						),
						x, y, z, 1, 0.2, 0.2, 0.2, 0.0
					);

					for (int j = 0; j < 4; j++) {
						Vec3 speed = new Vec3(
							(target.getRandom().nextFloat() - target.getRandom().nextFloat()) * 0.1F,
							target.getRandom().nextFloat() * 0.05F,
							(target.getRandom().nextFloat() - target.getRandom().nextFloat()) * 0.1F
						).normalize();

						ESPlatform.INSTANCE.sendToAllClients(
							serverLevel,
							new ParticlePacket(
			...
			if (target.addEffect(new MobEffectInstance(ESMobEffects.CRYSTAL_INFECTION.get(), 60, newLevel))) {
		...
		if (soundPitch >= 0 && !attacker.isSilent()) {
			attacker.level().playSound(
				...
				attacker.getY(),
				attacker.getZ(),
				ESSoundEvents.CRYSTAL_GREATSWORD_CHIME.get(),
