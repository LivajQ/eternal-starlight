...
public class ESFishingLootSubProvider implements LootTableSubProvider {
	@Override
		...
		consumer.accept(ESLootTables.GAMEPLAY_FISHING,
			LootTable.lootTable()
				.withPool(LootPool.lootPool().setRolls(ConstantValue.exactly(1.0F))
					.add(LootTableReference.lootTableReference(ESLootTables.GAMEPLAY_FISHING_JUNK).setWeight(10).setQuality(-2))
					.add(LootTableReference.lootTableReference(ESLootTables.GAMEPLAY_FISHING_TREASURE).setWeight(5).setQuality(2)
					...
					.add(LootTableReference.lootTableReference(ESLootTables.GAMEPLAY_FISHING_FISH).setWeight(85).setQuality(-1))));
			...
			LootTable.lootTable()
				.withPool(LootPool.lootPool()
					...
					.add(LootItem.lootTableItem(ESItems.LUMINOFISH.get()).setWeight(15)
						.when(LocationCheck.checkLocation(LocationPredicate.Builder.location().setBiome(ESBiomes.THE_ABYSS))))
			...
			LootTable.lootTable()
				.withPool(LootPool.lootPool()
					...
					.add(LootItem.lootTableItem(ESItems.SPIRAL_KELP.get())
						.when(LocationCheck.checkLocation(LocationPredicate.Builder.location().setBiome(ESBiomes.SPIRAL_KELP_FOREST)))
						.setWeight(10))
					.add(LootItem.lootTableItem(ESItems.JINGLESTEM_SAPLING.get())
						.when(LocationCheck.checkLocation(LocationPredicate.Builder.location().setBiome(ESBiomes.LUSH_SHALLOW_SEA)))
						.setWeight(10))));
		consumer.accept(
